...
class BsMemberSecurityCB extends AbstractConditionBean {
     ...
     * @return this. (NotNull)
     */
    def acceptPK(memberId: Integer): MemberSecurityCB = {
        assertObjectNotNull("memberId", memberId);
        val cb: BsMemberSecurityCB = this;
        cb.query().setMemberId_Equal(memberId);
        return this.asInstanceOf[MemberSecurityCB];
    }
     ...
     * DbleMemberSecurity memberSecurity = memberSecurityBhv.selectEntityWithDeletedCheck(cb);
     ...
     */
    def columnQuery(leftSpecifyQuery: (MemberSecurityCB) => Unit): ScrHpColQyOperand[MemberSecurityCB] = {
        return new ScrHpColQyOperand[MemberSecurityCB](new HpColQyHandler[MemberSecurityCB]() {
            def handle(rightSp: SpecifyQuery[MemberSecurityCB], operand: String): HpCalculator = {
                return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), new SpecifyQuery[MemberSecurityCB]() {
                    def specify(cb: MemberSecurityCB): Unit = { leftSpecifyQuery(cb); }
                }, rightSp, operand);
            }
        });
    }
     ...
     */
    def orScopeQuery(orQuery: (MemberSecurityCB) => Unit): Unit = {
        xorSQ(this.asInstanceOf[MemberSecurityCB], new OrQuery[MemberSecurityCB]{
            def query(orCB: MemberSecurityCB): Unit = { orQuery(orCB); }
        });
    }
     ...
     */
    def orScopeQueryAndPart(andQuery: (MemberSecurityCB) => Unit): Unit = {
        xorSQAP(this.asInstanceOf[MemberSecurityCB], new AndQuery[MemberSecurityCB] {
            def query(cb: MemberSecurityCB): Unit = { andQuery(cb); }
        });
    }
...
object HpMemberSecurityCB {
    ...
    class HpSpecification(baseCB: ConditionBean, qyCall: HpSpQyCall[MemberSecurityCQ], purpose: HpCBPurpose, dbmetaProvider: DBMetaProvider)
         ...
         */
        def myselfDerived(): ScrHpSDRFunction[MemberSecurityCB, MemberSecurityCQ] = {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return toScalaSDRFunction(new HpSDRFunction[MemberSecurityCB, MemberSecurityCQ](_baseCB, _qyCall.qy(), new HpSDRSetupper[MemberSecurityCB, MemberSecurityCQ]() {
