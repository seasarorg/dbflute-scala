 ...
 */
class BsSummaryProductCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected var _conditionQuery: SummaryProductCQ = null;
     ...
     */
    def query(): SummaryProductCQ = {
        assertQueryPurpose(); // assert only when user-public query 
        return getConditionQuery();
    }

    def getConditionQuery(): SummaryProductCQ = { // public for parameter comment and internal
    ...
    protected def createLocalCQ(): SummaryProductCQ = {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected def xcreateCQ(childQuery: ConditionQuery, sqlClause: SqlClause, aliasName: String, nestLevel: Integer): SummaryProductCQ = {
        val cq: SummaryProductCQ = xnewCQ(childQuery,  sqlClause,  aliasName,  nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected def xnewCQ(childQuery: ConditionQuery, sqlClause: SqlClause, aliasName: String, nestLevel: Integer): SummaryProductCQ = {
     ...
     */
    def union(unionQuery: UnionQuery[SummaryProductCB]): Unit = {
        val cb: SummaryProductCB = new SummaryProductCB(); cb.xsetupForUnion(this); xsyncUQ(cb); 
        try { lock(); unionQuery.query(cb); } finally { unlock(); }
        xsaveUCB(cb);
        val cq: SummaryProductCQ = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    def unionAll(unionQuery: UnionQuery[SummaryProductCB]): Unit = {
        val cb: SummaryProductCB = new SummaryProductCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionQuery.query(cb); } finally { unlock(); }
        xsaveUCB(cb);
        val cq: SummaryProductCQ = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    //                                                                             Specify
    //                                                                             =======
    protected var _specification: HpSummaryProductCB.HpSpecification = null;
     ...
     */
    def specify(): HpSummaryProductCB.HpSpecification = {
        assertSpecifyPurpose();
        if (_specification == null) { _specification = new HpSummaryProductCB.HpSpecification(this
            , new HpSpQyCall[SummaryProductCQ]() {
                def has(): Boolean = { return true; }
                def qy(): SummaryProductCQ = { return getConditionQuery(); }
            }
     ...
     */
    def columnQuery(leftSpecifyQuery: (SummaryProductCB) => Unit): ScrHpColQyOperand[SummaryProductCB] = {
        return new ScrHpColQyOperand[SummaryProductCB](new HpColQyHandler[SummaryProductCB]() {
            def handle(rightSp: SpecifyQuery[SummaryProductCB], operand: String): HpCalculator = {
                return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), new SpecifyQuery[SummaryProductCB]() {
                    def specify(cb: SummaryProductCB): Unit = { leftSpecifyQuery(cb); }
                }, rightSp, operand);
            }
        });
    }

    protected def xcreateColumnQueryCB(): SummaryProductCB = {
        val cb: SummaryProductCB = new SummaryProductCB();
        cb.xsetupForColumnQuery(this.asInstanceOf[SummaryProductCB]);
        return cb;
    }
     ...
     */
    def dreamCruiseCB(): SummaryProductCB = {
        val cb: SummaryProductCB = new SummaryProductCB();
        cb.xsetupForDreamCruise(this.asInstanceOf[SummaryProductCB]);
        return cb;
    }
     ...
     */
    def orScopeQuery(orQuery: (SummaryProductCB) => Unit): Unit = {
        xorSQ(this.asInstanceOf[SummaryProductCB], new OrQuery[SummaryProductCB]{
            def query(orCB: SummaryProductCB): Unit = { orQuery(orCB); }
        });
    }
     ...
     */
    def orScopeQueryAndPart(andQuery: (SummaryProductCB) => Unit): Unit = {
        xorSQAP(this.asInstanceOf[SummaryProductCB], new AndQuery[SummaryProductCB] {
            def query(cb: SummaryProductCB): Unit = { andQuery(cb); }
        });
    }
    ...
    protected def xprepareSyncQyCall(mainCB: ConditionBean): Unit = {
        val cb: SummaryProductCB = if (mainCB != null) {
            mainCB.asInstanceOf[SummaryProductCB];
        } else {
            new SummaryProductCB();
        }
        specify().xsetSyncQyCall(new HpSpQyCall[SummaryProductCQ]() {
            def has(): Boolean = { return true; }
            def qy(): SummaryProductCQ = { return cb.query(); }
        });
    }
    ...
    protected def getConditionBeanClassNameInternally(): String = { return classOf[SummaryProductCB].getName(); }
    protected def getConditionQueryClassNameInternally(): String = { return classOf[SummaryProductCQ].getName(); }
 ...
 * @author DBFlute(AutoGenerator)
 */
object HpSummaryProductCB {

    class HpSpecification(baseCB: ConditionBean, qyCall: HpSpQyCall[SummaryProductCQ], purpose: HpCBPurpose, dbmetaProvider: DBMetaProvider)
            extends HpAbstractSpecification[SummaryProductCQ](baseCB, qyCall, purpose, dbmetaProvider) {
        /**
         ...
         * @return The information object of specified column. (NotNull)
         */
        def columnLatestPurchaseDatetime(): HpSpecifiedColumn = { return doColumn("LATEST_PURCHASE_DATETIME"); }
        ...
        @Override
        protected def getTableDbName(): String = { return "SUMMARY_PRODUCT"; }
    }
